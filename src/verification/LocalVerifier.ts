import { Checkpoint, GameEvent } from '../chain/types.ts';
...
export class LocalVerifier {
    ...
    public verifyChain(checkpoints: Checkpoint[], sessionId: string): VerificationResult {
        console.log(`   Verifying complete chain...`);
        ...
        if (checkpoints[0].previousHash  !== expectedGenesisHash) {
            console.log(`   Genisis hash mismatch`);
            return {
                valid: false,
            ...
            if (event.type === 'PLAYER_MOVE' && event.data.position) {
                if (lastPosition) {
                    ...
                    if (speed > maxSpeedPerMs) {
                        ...
                        return {
                            valid: false,
                            message: `An impossible movement speed at event ${i}. Possible speed hack.`,
                            failedAtEvent: i,
                        };
                    }
                }
    ...
    // Full verification: hash chain + game rules
    ...
    public fullVerification(checkpoints: Checkpoint[], sessionId: string): VerificationResult {
        ...
        return {
            valid: true,
            message: `Full verification pased: hash chain and game rules valid.`,
        };
    }
}